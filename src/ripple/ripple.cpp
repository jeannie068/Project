...
int Ripple::getArgs(int argc, char** argv) {
    ...
    for (int a = 1; a < argc; a++) {
        if (strcmp(argv[a], "-bookshelf") == 0) {
        ...
        else if (!strcmp(argv[a], "-groupFile")) {     // for TYPE4 cell group file generated by BOTI's GP engine
            io::IOModule::GroupFile.assign(argv[++a]);
        }
#ifdef __GP__
            ...
            /*----------modded----------*/
             else if (!strcmp(argv[a], "-IgnorePinAccFence")) {
             ...
             else if (!strcmp(argv[a], "-onlyMGL")) {
                printlog(LOG_INFO, "only MGL");
                dp::DPModule::onlyMGL=1;
            }
             else if (!strcmp(argv[a], "-refinement")) {
                printlog(LOG_INFO, "Refinement mode");
                dp::DPModule::refinementMode=1;
            }
            /*----------modded----------*/
             else {
