...
bool Region::independent(const Container& c) const {
    for (const auto& [cell, r] : c) {
        ...
        if (r.lx <= hx && r.hx >= lx && r.ly <= hy && r.hy >= ly) { //region被包在c裡面
            return false; 
        }
    }
    return true;
...
public:
    ...
    int neededSpace = 0;    // needed space for the cells in this local interval

    LegalMoveInterval(const int row = 0) : r(row) {}

    float optPointL() const { return _optPointL; }  // right-boundary of the left cell
    float optPointR() const { return _optPointR; }  // left-boundary of the right cell

    void optPointL(const float l) { _optPointL = l; }   // right-boundary of the left cell
    void optPointR(const float r) { _optPointR = r; }   // left-boundary of the right cell
...
public:
    Cell* parent = nullptr;     // LocalCell's database in the whole floorplan
    int i = -1; // cell's index in pos list
    int x = -1;
    int y = -1;
    int w = 0;
    unsigned h = 0;
...
public:
    ...
    int hx = -1;
    int hy = -1;
    vector<LocalSegment> localSegments; // local segment = cut the region row by row. It's not the same definition as the segemnt in the whole region
    vector<LocalCell> localCells;
    vector<int> localCellMap;
    vector<int> localCellList;
    unsigned nSegments; // local region cut into #row of segments
    unsigned nCells;
    ...
    void reset();
    void clear();
    void placeL(vector<int>& posL, const vector<CellTypeMap<MLLCost>>& cellTypeMaps);   // place the left cell next to the target cell
    void placeR(vector<int>& posR, const vector<CellTypeMap<MLLCost>>& cellTypeMaps);   // place the right cell next to the target cell
    void placeSpreadL(int targetCell, vector<int>& posL, const vector<CellTypeMap<MLLCost>>& cellTypeMaps); // spread all the remain left cells next to the target cell's left cell in the region
    void placeSpreadR(int targetCell, vector<int>& posR, const vector<CellTypeMap<MLLCost>>& cellTypeMaps); // spread all the remain right cells next to the target cell's right cell in the region
...
public:
    // give a cell with an available operating region   
    // pair<Cell*, Region>
    ConcurrentQueue<pair<Cell*, Region>>* tQ = nullptr;
    // after legaling a cell, push the cell into rQ
    // legalized: false, not legalized: true
    // pair<Cell*, bool>
    ...
    map<int, list<shared_ptr<Cell>>> rows;
