...
double DPlacer::defineLocalRegion(LocalRegion& localRegion, Cell* targetCell, const Region& region) {
    ...
    const unsigned maxLocalCells = std::min((unsigned)((hx - lx) * (hy - ly)), (unsigned)cells.size()); // maximum number of cells can be placed on this region
    const unsigned oldSize = localRegion.localSegments.size();
    if (oldSize < nRows) {  // at least ther are # nRows of segments
    ...
    for (int ry = ly; ry != hy; ++ry) {
        unsigned r = ry - ly;   // local region's row index

        // local segment = cut the region row by row
        LocalSegment& localSeg = localRegion.localSegments[r];
        localSeg.x = lx;    // local segment left right boundary = region's boundary
        localSeg.y = ry;
        localSeg.w = hx - lx;
        ...
        for (Segment& seg : segments[ry]) {
            // seg = segment in the whole floorplan. The segment in the while floorplan may not occupy the whole row
            if (seg.boundR() <= lx) {
                continue;
            }
...
void DPlacer::getDBDisplacement(long long& sum, vector<double>& mean, unsigned long long& max, const Cell*& maxCell) {
    ...
    for (const Cell* cell : cells) {
        ...
        // if (cell->h > maxH) {
        //     cout << "error cell: " << cell->getDBCell()->name() << endl;
        // }
        assert(cell->h <= maxH);
        dist[cell->h - 1] += disp;
        if (disp > max) {
