            ...
            while (fronts[r] != ends[r]) {
                ...
                if (cell.h == 1) {
                    ...
                    for (unsigned s = 1; s < cell.h; ++s) {
                        // the current X position in other row(segment) is not the same as current cell
                    ...
                    if (blocked) {
                        ...
                        // the cell is not blocked, find a good position for this cell
                        // choose an X coordinate on different row(segment) that has the largest X coordinate
                        // which has the minimum impact for other left cell
                        int lb = LBounds[r];
                        for (unsigned s = 1; s < cell.h; ++s) {
                    ...
                    if (fronts[r + s] + 1 != ends[r + s]) {
                        ...
                        if (parent) {
                            ...
                            if (cell.fixed) {
                                ...
                                // parent = localCells[localSegments[r + s].localCells[fronts[r + s] + 1]].parent
                                // cell.parent = localCells[localSegments[r].localCells[fronts[r]]].parent
    ...
    while (!finished) {
        ...
        for (unsigned r = 0; r < nSegments; r++) {
            ...
            for (int i = nCells - 2; i >= 0; i--) {
                ...
                LocalCell* cellR = &localCells[segment.localCells[i + 1]];  // previously moved
    ...
    while (!finished) {
        ...
        for (unsigned r = 0; r < nSegments; r++) {
            ...
            for (int i = 1; i < nCells; i++) {
                ...
                LocalCell* cellL = &localCells[segment.localCells[i - 1]];  // previously moved
    ...
    while (!finished) {
        ...
        for (const auto& segment : localSegments) {
            for (int i = segment.nCells - 2; i >= 0; i--) {
                ...
                // if (Cell::getSpace(cellL->parent, cellR->parent) > 0) cout << Cell::getSpace(cellL->parent, cellR->parent) << endl;
                int lW = cellL->w + Cell::getSpace(cellL->parent, cellR->parent);
                if (posL[cellL->i] < 0) {
