...
int Cell::getSpace(const Cell* lCell, const Cell* rCell) {
    // return getSpace(lCell->type, lCell->_fx, rCell->type, rCell->_fx);
    if ((lCell->h == 4 && rCell->h == 4)) 
    {
        // Two TYPE4 cell no need to have horizontal separation
        // overlapping fake cell area
        // return 1;
        return -2;
    }
    else
    {
        // other cells no need to have horizontal separation
        return 0;
    }
}
...
unsigned Segment::setLocalRegion(LocalRegion& localRegion, unsigned& nLocalCells, const int fid) {
    ...
    // use fake cell to fill up the left boundary of local segment
    if (ci != cells.begin()) {
    ...
    for (; ci != ce; ++ci) {
        ...
        // overlap cells (cell is larger than the whole local segment)
        ...
        // use fake cell to fill up the right boundary of local segment
        ...
        // add this cell to local segment
        } else {
